...
def save_confusion_matrix_plot(y_test, y_pred, save_path: str) -> str:
    ...
    try:
        ...
        disp = ConfusionMatrixDisplay(
            confusion_matrix=cm, display_labels=["Not Satisfied", "Satisfied"]
        )
...
def main():
    try:
        ...
        if target_col not in test_data.columns:
            raise ValueError(
                f"Target column '{target_col}' not found in processed test data. Available columns: {list(test_data.columns)}"
            )
        ...
        with mlflow.start_run(run_id=run_id):
            ...
            cm_plot_path = save_confusion_matrix_plot(
                y_test, y_pred, save_path="reports/plots/confusion_matrix.png"
            )
            mlflow.log_artifact(cm_plot_path, artifact_path="plots")

            # ROC curve plot
            roc_plot_path = save_roc_curve_plot(
                model, X_test, y_test, save_path="reports/plots/roc_curve.png"
            )
